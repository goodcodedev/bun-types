...
- [PrivateKeyInput](../interfaces/crypto._node_crypto_.PrivateKeyInput.md)
...
- [createSign](crypto._node_crypto_.md#createsign)
...
returned object mimics the interface of objects created by [createDiffieHellman](crypto._node_crypto_.md#creatediffiehellman), but will not allow changing
...
An array of supported digest functions can be retrieved using [getHashes](crypto._node_crypto_.md#gethashes).
...
the corresponding public key, for example using [publicEncrypt](crypto._node_crypto_.md#publicencrypt).

If `privateKey` is not a `KeyObject`, this function behaves as if`privateKey` had been passed to [createPrivateKey](crypto._node_crypto_.md#createprivatekey). If it is an
...
the corresponding public key, for example using [publicDecrypt](crypto._node_crypto_.md#publicdecrypt).
...
the corresponding private key, for example using [privateEncrypt](crypto._node_crypto_.md#privateencrypt).

If `key` is not a `KeyObject`, this function behaves as if`key` had been passed to [createPublicKey](crypto._node_crypto_.md#createpublickey). If it is an
object, the `padding` property can be passed. Otherwise, this function uses`RSA_PKCS1_PADDING`.
...
the corresponding private key, for example using [privateDecrypt](crypto._node_crypto_.md#privatedecrypt).
...
This function is similar to [randomBytes](crypto._node_crypto_.md#randombytes) but requires the first
...
Synchronous version of [randomFill](crypto._node_crypto_.md#randomfill).
...
passed to [createPrivateKey](crypto._node_crypto_.md#createprivatekey). If it is an object, the following
...
If `key` is not a `KeyObject`, this function behaves as if `key` had been
passed to [createPublicKey](crypto._node_crypto_.md#createpublickey). If it is an object, the following
additional properties can be passed:
